...
the number the squared divisors of which is a square and then the sum of the squared divisors.
...
function listSquared(m, n) {
  ...
  for (let i = m; i <= n; i++) {
    const dev = getDivisors(i);
    const sum = dev.reduce((acc, item) => acc + item * item, 0);
